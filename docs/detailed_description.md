...
#### Optional inputs
- **path_parcellation_table**, path to a Parcel object that assigns each region of interest (ROI) to a given network. It can be provided as a [dot mat](./detailed_specs/HCP_ColeAnticevic.mat) file ot as a table in [csv format](./detailed_specs/HCP_ColeAnticevic.csv).
